...
| `ensure_block_locale` | For structure blocks: `(structure_block_id, language)` (the URL `{localeId}` segment takes the language code — uppercased server-side — despite its name, NOT an integer). For content blocks: `(content_block_id, language[, version])` — the route returns an integer `locale_id` you keep for subsequent updates | **Structure block locale:** `PUT /api/v1/documents/templates/{template_id}/structure/blocks/{block_id}/locales/{localeId}` with body `{title}` (the `{localeId}` segment is the language code, not an integer). **Content block locale:** `POST /api/v1/documents/content-blocks/{block_id}/locales` with body `{language, version?, blocks?, template_name?, is_active?}` to upsert (mutually exclusive: send either `blocks` — array of EditorJS blocks per `rattle-techdoc/references/editorjs-blocks.md` — OR `template_name` for dynamic slots). For subsequent edits use `PUT/PATCH /api/v1/documents/content-blocks/{block_id}/locales/{localeId}` with the integer id from the create response. |
