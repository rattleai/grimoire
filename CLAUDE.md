...
| `skills/rattle-i18n/` | **Translate · locale-resolve · glossary-lock.** 25 API ops (languages, `/translations`, the `/translations/dictionary` glossary, DeepL-backed block + template translate, `is_stale` staleness). **The core rule: every string is in exactly one of three buckets, and confusing them is the failure.** Free prose → machine-translate. **Regulated text — CLP H/P statement text, ISO 3864/ANSI signal words — is LOCALE-RESOLVED from official tables and MUST NEVER be AI-translated** (a DeepL'd CLP statement is a legal defect in a CE-marked document, not a typo). Brand/domain terms → glossary-locked. |
| `skills/rattle-pricing/` | **Price lists, overrides, advanced (conditional) prices, presets.** 37 API ops. **Five mechanisms can set one option's price and Rattle does not document which wins** — so this skill asserts **no precedence table** and instead teaches empirical determination via `POST /configurations/calculate` (the API's own price oracle), recording the observed order in tenant memory. Also documents `advanced-prices`, an undocumented cross-option conditional-price engine (`condition_option_id`) whose schemas are inline and unnamed. |
| `skills/rattle-crm-quotes/` | **The money end.** Customer → contacts → opportunity → configuration → **finalize (lock it)** → quote → line items → commercials → document → status → revise. 49 API ops that had zero coverage. Encodes: `finalize` before quoting (a line pointing at a mutable configuration is a quote that can silently change after you send it), `price_list_id` is required on every quote, discounts are **update-only** (POST then PATCH), and the quote/opportunity **status vocabulary is a free string with no enum — never invent one; read the tenant's**. |
