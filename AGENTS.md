...
| `skills/rattle-i18n/SKILL.md` | Translation, localisation, the glossary lock. **Load before translating anything.** The rule: free prose → machine-translate (DeepL); **regulated text (CLP H/P statements, ISO signal words) → LOCALE-RESOLVED, NEVER AI-translated** — machine-translating one is a legal defect in a CE-marked document; brand terms → glossary-locked via `/translations/dictionary`. Check `is_stale` before shipping. |
| `skills/rattle-pricing/SKILL.md` | Price lists, price overrides, advanced (conditional) prices, pricing presets. **Load before touching any price.** Critical rule: **five mechanisms can set one option's price and Rattle does not document which wins** — never assert a precedence order; determine it empirically per tenant with `POST /configurations/calculate` and record it. |
| `skills/rattle-crm-quotes/SKILL.md` | **The money end** — quote-to-cash. Customer → opportunity → configuration → **finalize** → quote → line items → document → status → revise. Load whenever a configuration has to turn into a quote. Key rules: finalize a configuration BEFORE quoting it; `price_list_id` is required on every quote; discounts are update-only; **the status vocabulary has no enum — read the tenant's, never invent one.** |
