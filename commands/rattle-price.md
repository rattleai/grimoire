...
8. **`/replace` is a bulk atomic wipe — and it is the dangerous one** — *"Delete all existing overrides and replace with the provided set."* `info.description` says bulk-replace uses **`X-Price-Lists-Version`** for optimistic locking. The current spec declares that header on **price-list CRUD** and `PriceListResponse` **now carries a `version`** to read it from — **but not one price-override `/replace` endpoint declares any header parameter, and none declares a `409`** (audit § **P0-1**). So the very operation that wipes overrides still has no declared way to lock it.

   > **A concurrent `/replace` silently destroys another user's overrides with a `200 OK`.**

   **Never run one without explicit human confirmation naming the tenant.** Prefer the granular POST/PATCH/DELETE path — it touches one row; `/replace` touches all of them. Documented rate limit: **`Price override replace | 30/minute`**. `429` responses now declare **`Retry-After`** (audit § **P3-3**, resolved) — honour it, and fall back to exponential backoff.
