...
**Now documented (current spec):** an advanced price **outranks and replaces an option price-override**, and omitting `area_id` / `price_list_id` (or sending `null`) means **applies to all**. **Still NOT known, and must not be guessed:** what happens when **two** advanced prices on the same option both match (two condition options both selected) — no tie-break is stated — and how an advanced price ranks against a *product* override, a preset, or the base price. Measure it or say you do not know. Full treatment, worked examples, and the update semantics: **`references/advanced-prices.md`**.
...
| # | Trap | Consequence |
|---|---|---|
| **P0-1** | **`/replace` is a bulk atomic wipe, and its OCC header is undeclared *on the endpoints that need it*.** `POST …/price-overrides/replace` — *"Delete all existing overrides and replace with the provided set."* `info.description` says bulk-replace uses **`X-Price-Lists-Version`** for optimistic locking. The current spec **does** now declare that header — but only on **price-list CRUD** (`POST/PATCH/PUT/DELETE /price-lists`); **not one price-override `/replace` endpoint declares any header parameter, and none declares a `409`**. | **A concurrent `/replace` silently destroys another user's overrides with a `200 OK`.** No error, no warning, no conflict. `PriceListResponse` **now carries a `version` field** — so *"read the current version from a GET response"* is finally followable for price-list writes — but the `/replace` endpoints expose no slot to send it in and no conflict to catch, so a spec-driven client still cannot lock them. Send the header where it is declared; **never batch a `/replace` you have not confirmed with a human.** Documented rate limit: **"Price override replace \| 30/minute"**. |
...
| **P0-10** | **The area price-override and option `area-config` bodies are still INLINE and do not set `additionalProperties: false`.** The option/product override **and now the advanced-price** bodies are named schemas that **do** (the advanced-price body was fixed upstream — see `references/advanced-prices.md` § 6). | **In the same resource family, a typo'd field `422`s on one endpoint and is swallowed with a `200` on the next.** An agent that learned "a bad field errors" is wrong exactly on the two remaining inline bodies. **Read every area-override and `area-config` write back and diff it.** |
| — | **Price-override keying is create-only; advanced-price scoping is not.** `PriceOverrideUpdateRequest` = `{override_price}` — you **cannot** re-point an option/area/product override at a different area or price list; **DELETE and re-POST.** The advanced-price `PUT` (there is no `PATCH` any more) **does** accept `area_id`/`price_list_id` (nullable — `null` widens to "all"), so only re-pointing the `condition_option_id` (`readOnly`) needs DELETE + re-POST. | **Know which fields each verb actually writes.** An update that silently ignores a field it does not accept returns `200` and changes nothing — you think you moved the override; you did not. |
...
| `ensure_advanced_price` | (option_id, condition_option_id, area_id, price_list_id) | `GET /options/{id}/advanced-prices` (**not paginated**) → `POST` / `PUT …/{priceId}` (no `PATCH`). Named schema, `additionalProperties: false` — a typo now `422`s. `PUT` re-scopes `area_id`/`price_list_id`; only `condition_option_id` is create-only. **Verify the price on `/calculate`.** |
